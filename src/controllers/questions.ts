...
class QuestionsCtl {
  ...
  async checkQuestionExist(ctx: any, next: any) {
  ...
  async checkQuestioner(ctx: any, next: any) {
  ...
  async informationStatistics(ctx) {
    ...
    const me: any = await User.findById(ctx.state.user._id).select('+followingQuestions');
