...
class UsersCtl {
    ...
    async create(ctx) {
        ...
        const user = await new users_1.default({ name, password, email }).save();
        ctx.body = user;
    }
    ...
    async update(ctx) {
        ...
        if (ctx.request.body.password) {
            // 密码加密单独处理
            const user = await users_1.default.findById(ctx.params.id).select('+password');
            user.password = ctx.request.body.password;
            delete ctx.request.body.password;
            await user.save();
        }
        const user = await users_1.default.findByIdAndUpdate(ctx.params.id, ctx.request.body);
        if (!user) {
            ctx.throw(404, '用户不存在');
        }
        ctx.body = user;
    ...
    async delete(ctx) {
        const user = await users_1.default.findByIdAndRemove(ctx.params.id);
        if (!user) {
            ctx.throw(404, '用户不存在');
        }
        ctx.status = 204;
    ...
    async logout(ctx) {
        ...
        await token_1.default.findOneAndRemove({ token });
        ctx.status = 204;
    }
    ...
    async listFollowers(ctx) {
        // 粉丝列表
        const users = await users_1.default.find({ following: ctx.params.id });
        ctx.body = users;
    }
    ...
    async follow(ctx) {
        const me = await users_1.default.findById(ctx.state.user._id).select('+following');
    ...
    async listQuestions(ctx) {
        // 列出用户提出问题
        const questions = await questions_1.default.find({ questioner: ctx.params.id });
        ctx.body = questions;
    }
    ...
    async likeAnswer(ctx, next) {
        // 赞答案
        const me = await users_1.default.findById(ctx.state.user._id).select('+likingAnswers');
        if (!me.likingAnswers.map((id) => id.toString()).includes(ctx.params.id)) {
            me.likingAnswers.push(ctx.params.id);
            me.save();
            await answers_1.default.findByIdAndUpdate(ctx.params.id, { $inc: { voteCount: 1 } });
        }
        ctx.status = 204;
        await next();
    }
    ...
    async disLikeAnswer(ctx, next) {
        // 踩答案
        const me = await users_1.default.findById(ctx.state.user._id).select('+dislikingAnswers');
    ...
    async collectAnswer(ctx) {
        // 收藏答案
        const me = await users_1.default.findById(ctx.state.user._id).select('+collectingAnswers');
    ...
    async likePeriodical(ctx, next) {
        // 赞期刊
        const me = await users_1.default.findById(ctx.state.user._id).select('+likingPeriodicals');
        if (!me.likingPeriodicals.map((id) => id.toString()).includes(ctx.params.id)) {
            me.likingPeriodicals.push(ctx.params.id);
            me.save();
            await periodical_1.default.findByIdAndUpdate(ctx.params.id, { $inc: { voteCount: 1 } });
        }
        ctx.status = 204;
        await next();
    }
